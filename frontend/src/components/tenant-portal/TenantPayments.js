      ...
      <div className="card">
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {payments.length === 0 ? (
                <tr><td colSpan="8" style={{ textAlign: 'center', padding: 24 }}>No payments found</td></tr>
              ) : (
                payments.map(p => (
                  <tr key={p.id}>
                    <td>{p.paymentDate}</td>
        ...
        <div className="modal-overlay" onClick={() => setReceipt(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gap: 10, fontSize: '0.9rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#718096' }}>Date:</span><span>{receipt.date}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#718096' }}>Unit:</span><span>{receipt.unit}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#718096' }}>Period:</span><span>{receipt.month} {receipt.year}</span>
              </div>
              <hr style={{ border: 'none', borderTop: '1px dashed #e2e8f0' }} />
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#718096' }}>Amount:</span>
                <span style={{ fontWeight: 700, color: '#38a169' }}>{formatCurrency(receipt.amount)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
